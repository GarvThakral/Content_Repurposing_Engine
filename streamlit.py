...
from helpers.logic import get_openrouter, prompt_to_generate_media , extract_content
from helpers.classes_varibales import MediaOutput , multi_media_to_generate
from helpers.logic import generate_content
demo_instructions = {
...
if content_type_dropbox == "Youtube Video":
    ...
    input_context = st.text_area("Enter your content as plain text !", value="", height=None, max_chars=None, key=None, help=None,
                                 on_change=None, args=None, kwargs=None, placeholder=None, disabled=False, label_visibility="visible", width="stretch")
prompt = st.text_area("Enter your prompt for content generation (This applies to every platform selected below)", value="", height=None, max_chars=None, key=None, help=None,
                                 on_change=None, args=None, kwargs=None, placeholder="Generate a post for instagram and twitter highlighting the key events of my new podcast with .......", disabled=False, label_visibility="visible", width="stretch")


st.title("Pick the platforms you want to generate content/scripts for .",
         anchor=None, help=None, width="stretch")
...
for option in multi_media_to_generate:
    ...
    if selected.get(option):
        selected_instructions[option] = st.text_area(option[0].upper() + option + " Post Instructions", value="", height=None, max_chars=None, key=None,
        help=None, on_change=None, args=None, kwargs=None, placeholder=placeholders[option], disabled=False, label_visibility="visible", width="stretch")

button_gen = st.button("Generate content", key=None, help=None, on_click=None, args=None, kwargs=None,
                       type="primary", icon=None, disabled=False, use_container_width=None, width="content")
...
if(content_generated):
  ...
  with c1:
      if st.button("⬅️ Prev"):
          st.session_state.carousel_idx = (
              st.session_state.carousel_idx - 1) % len(selected_types)
  with c3:
      if st.button("Next ➡️"):
          st.session_state.carousel_idx = (
              st.session_state.carousel_idx + 1) % len(selected_types)
  ...
  # Render
  html = html_snippets.get(
      ctype, "<div style='padding:16px'>No HTML for this type.</div>")
